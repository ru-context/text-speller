...
from torch.nn import functional as F  # Импорт functional
...
class Config:
    input_size = None
    hidden_size = 64  # Уменьшил hidden_size
    output_size = None
    learning_rate = 0.001
    num_epochs = 10
    batch_size = 16  # Уменьшил batch_size
...
# Векторизация текста
vectorizer = TfidfVectorizer(analyzer='char', ngram_range=(2, 3), max_features=5000)  # Уменьшил размерность
...
class SpellCorrectionModel(nn.Module):
    ...
    def forward(self, x):
        x = F.relu(self.fc1(x))  # Используем F.relu
        x = self.dropout(x)
        x = F.relu(self.fc2(x))
        x = self.dropout(x)
        x = self.fc3(x)
...
for epoch in range(Config.num_epochs):
    ...
    logging.info(f"Эпоха [{epoch + 1}/{Config.num_epochs}], Loss: {running_loss / len(train_loader):.4f}")
